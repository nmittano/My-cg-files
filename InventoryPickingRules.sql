...
WHERE
      ...
      ---and wsc.from_organization_id = WRV.RULE_ID   ----wsc.vn_from_organization_id
       ----AND wsc.rule_type_code         = 2;
...
ORDER BY 1,SEQUENCE_NUMBER;
      ...
      ---AND WSCT.from_organization_id = '5703'
      --AND WSCT.RETURN_TYPE_NAME  = WRV.NAME
       ---AND WSCT.from_organization_id = '5703'
      ---AND WSCT.rule_type_code         = 2
      
      
      SELECT * FROM MTL_INV_PICKING_RULES;
      SELECT * FROM WMS_SELECTION_CRITERIA_TXN_V;
      SELECT * FROM WMS_RULES_VL;
      select * from MTL_PARAMETERS;
      select * from MTL_ATP_RULES;
